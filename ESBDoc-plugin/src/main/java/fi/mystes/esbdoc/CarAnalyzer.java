    ...
    private Set<String> evaluateXPathToStringSet(XdmNode context, XPathSelector xpath) throws SaxonApiException {
        if(null == xpath){
            if(log.isDebugEnabled())
                log.debug("Cannot apply XPath to Context: XPath is null! Returning empty HashSet...");
            return new HashSet<String>();
        }

        if(null == context){
            if(log.isDebugEnabled())
                log.debug("Cannot apply XPath to Context: Context is null! Returning empty HashSet...");
            return new HashSet<String>();
        }
